...
// hostInternal pinned-empty sweep (cinatra#172 Stage H4): account/settings/
// status reads resolve the host-bound deps slot (bound at serverEntry
// activation by `register(ctx)` adapting `@cinatra-ai/host:linkedin-connection`)
// instead of importing `@/lib/linkedin-api`. Account rows arrive
// token-stripped by contract — this page never needed token material.
...
export async function LinkedInSettingsPage({ searchParams }: SettingsLinkedInPageProps) {
  const [accounts, settings, status, resolvedSearchParams] = await Promise.all([
    getLinkedInDeps().listAccounts(),
    getLinkedInDeps().getSettings(),
    getLinkedInDeps().getStatus(),
